 ...
 */
  void build_vect2(float* x,int vector_size,int deb,int step,int coef,float* res,int deb_res,int step_res)
  {
    //( n,x, incx, y, incy)
    //res=x
    // deb and deb_res are typically shifts ( example when we want to do a treatment on vector begining with 2nd element , we just set deb=1 )
    cblas_scopy(vector_size,(x+deb),step,(res+deb_res),step_res);
    if (coef != 0)
      //( n,  alpha, x, incx) 
      //res=coef*res (with a deb_res shift if exists)
      cblas_sscal(vector_size,coef,(res+deb_res),step_res);
/* this function calculates res= coef*x vectorized with a possible shift on res and x*/
  }
